...
* **Cloud Manager**
   ...
   * パイプラインウィザードは、Experience Manager 環境のアップデートが必要な場合、それに関連付けられた [Web Tier Config パイプライン](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/using-cloud-manager/cicd-pipelines/introduction-ci-cd-pipelines.html?lang=ja#web-tier-config-pipelines)を設定する前に、ユーザーに通知します。
...
| 2022年4月 | [DDX 操作の呼び出しを使用した PDF のアセンブリ](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/forms/forms-cs-assembler/introduction.html) | ビデオ | リポジトリブラウザーは、AEM の基盤となるデータストアを可視化する強力なツールで、AEM as a Cloud Service 環境のデバッグを容易にします。リポジトリブラウザーは、実稼動、ステージ、開発に加え、オーサー、パブリッシュおよびプレビューサービスの、AEM のすべてのリソースおよびプロパティの検査をサポートします。 | AEM CS |
| 2022年4月 | [レターインスタンスを保存および取得するための実装ソリューション](https://experienceleague.adobe.com/docs/experience-manager-learn/forms/store-retrieve-letters/introduction.html) | ビデオ | SPI を使用して、インタラクティブコミュニケーションのレターインスタンスを保存および取得するための実装方法を説明します。 | AEM Forms |
...
| 2022年4月 | [監査証跡](https://experienceleague.adobe.com/docs/campaign-learn/tutorials/monitoring/audit-trail.html) | ビデオ | 監査証跡ログにアクセスする方法と設定可能な構成について説明します。 | Campaign v8 |
