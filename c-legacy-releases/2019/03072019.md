...
| 中国の地方加速 | Analyticsレポートスイートの設定で「中国のアクセラレーション」オプションが使用できるようになりました。 このオプションを選択すると、中国本土のユーザーに対するアドビのAnalyticsレポートユーザーエクスペリエンスが向上します。 各AdobeAnalyticsユーザーは、このオプションを有効または無効にできます。 パフォーマンスに悪影響が出るので、中国本土以外のユーザーに対してはこのオプションを選択しないでください。 [詳細情報](https://marketing.adobe.com/resources/help/ja_JP/sc/user/report_settings.html) |
...
* Analytics の（移行後の）ユーザーの氏名が Experience Cloud 内でユーザーの Adobe ID にリンクされた名前と一致しないことがある問題を修正しました。Analytics ユーザー名を Experience Cloud 上の名前に更新するには、「**[!UICONTROL 管理**／**!UICONTROL ユーザー管理（レガシー）**／**!UICONTROL ユーザーの編集**」へ移動して、ユーザー ID をクリックします。次に、「 **!UICONTROL Experience Cloudから&#x200B;**更新」をクリックします。 （AN-172825）
...
このリリースでは、Adobe Campaignと、Adobe Experience Platform起動およびMobile SDKのAdobe Campaign Standard用の一般公開(GA)バージョンのAndroidおよびiOS拡張機能との統合を説明します。 これらの拡張機能は、プッシュメッセージ、アプリ内メッセージおよびモバイルアプリプロファイルの更新をサポートしています。
...
Cloud Manager 2019.2.0のリリースには、システム監視機能が追加されています。 この新機能により、お客様はAdobe Managed Services環境の状態をシステムレベルで表示できます。
...
詳しくは、「AEM Assets [のブランドポータル](https://helpx.adobe.com/jp/experience-manager/brand-portal/using/whats-new.html) / [リリースノートの新機能](https://helpx.adobe.com/jp/experience-manager/brand-portal/release-notes/brand-portal-release-notes.html)」を参照してください。
...
* 「もっと読み込む」をクリックすると、一部の製品IDでエラーが発生する問題を修正しました
