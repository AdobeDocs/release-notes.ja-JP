...
| **Analysis Workspace：**&#x200B;新しい「_繰り返しインスタンスを含める_」フロービジュアライゼーション設定 | 「繰り返しインスタンス _を含む_ 」フロー設定では、ページ再読み込みなどの繰り返しインスタンスを含めるか除外するかを選択できます。 さらに、すべてのフロービジュアライゼーションは、インスタンスにのみ基づいています。 |
| **Ad Hoc Analysis：** Java 11 との互換性 | Ad Hoc Analysis は、Java 11 をサポートするようになりました。Java 11 [上で実行するAd hoc analysis方法を説明します](https://marketing.adobe.com/resources/help/en_US/dsc/adhoc-java.html)。 |
| **データ収集：** 新しい s_ecid cookie | Added a new [first-party server cookie](https://docs.adobe.com/content/help/ja-JP/core-services/interface/ec-cookies/cookies-analytics.html), s_ecid, in which data collection stores the visitor&#39;s ECID. |
...
* オーディエンスAnalytics: Audience Manager(AAM)でオーディエンス名が変更された後に発生する問題を修正しました。オーディエンスAnalyticsには更新された名前が反映されませんでした。 （AN-176237）
* ユーザーがAAMにAnalyticsセグメントを保存できない問題を修正しました。 これは、大文字と小文字の名前が混在する既存のAAMフォルダーが原因です。 現在は、すべてのフォルダーで大文字と小文字が区別されず、同期が行われます。 （AN-177934）
* ユーザーがExperience Cloud経由でAnalyticsにログインし、その後セッションがタイムアウトした場合に発生していた問題を修正しました。 セッションを再開する際に、ユーザーが障害のあるURLにリダイレクトされていた問題を修正しました。 （AN-176812）
...
| Analysis Workspaceデバッガーコマンドの今後の変更 | 2019 年 4 月 5 日 | Analysis Workspace Debugger をオンにするためのコンソールコマンドは、**2019 年 6 月 14 日**、adobeTools.debug.includeOberonXml に変更されます。 |
...
| 短いAnalyticsレポートリンク | 2019 年 1 月 15 日 | 2019 年 1 月 17 日（木）以降、直近 1 年間に訪問されていない短い Analytics レポートリンクは、定期的に削除されるようになります。 |
...
| アカウントアクティビティレポートの削除予定について | 2018 年 5 月 11 日 | アカウントアクティビティレポートは、AdobeAnalytics夏リリースのサーバーコールの使用機能に置き換えられます。 アカウントアクティビティレポートは、2018 年 8 月 9 日に完全に削除されます。2018 年 8 月 9 日以降は、レポートスイートのトラフィックに関するサマリデータを表示する場合、サーバーコール使用状況の機能をご使用ください。 |
...
AEM 6.5（2019 年 4 月 8 日から利用可能）は、AEM 6.4 コードベースに対するアップグレードリリースです。AEM 6.5の最新のアップデートにより、お客様のビジネスをより迅速に進める画期的な改善点にすぐにアクセスできます。
...
AEM 6.4、Service Pack 4（2019年4月4日にリリースされた6.4.0）は重要なアップデートで、2018年4月のAEM 6.4の一般リリース以降にリリースされた主なお客様向け修正が含まれています。

[AEM 6.4 Service Pack](https://helpx.adobe.com/jp/experience-manager/6-4/release-notes/sp-release-notes.html)[AEM Formsリリースのリリースノート](https://helpx.adobe.com/jp/em-forms/kb/aem-forms-releases.html)

**AEM S3 Connector**

2019年6月24日に署名バージョン2のサポートが終了すると、S3へのアクセスエラーが発生したため、S3データストアコネクタの古いバージョンを含むAEMインスタンスが使用できなくなる場合があります。 AEMのお客様は、使用しているS3 Datastoreコネクタのバージョンを確認することをお勧めします。 必要に応じて、最新バージョンに更新します。

Amazon S3 [のAWS Signature Version 2非推奨の影響を参照](https://helpx.adobe.com/jp/experience-manager/kb/the-impact-of-aws-signature-version-2-deprecation-for-amazon-s3.html)

### セルフサービス

**AEM Sites コードベースの最新化**

最新の AEM テクノロジーを利用して AEM Sites コードベースを最新化する方法を説明します。[既存のAdobe Experience Managerサイトのコードベースの最新化](https://expleague.azureedge.net/labs/L761/index.html)

**AEMリッチテキストエディタ — 詳細**

AEMのリッチ設定とリッチテキストエディタの使用に関するベストプラクティスを説明します。

詳しくは、 [AEM Rich Text Editor(RTE)の詳細を参照してください。](https://helpx.adobe.com/jp/experience-manager/kt/eseminars/gems/AEM-Rich-Text-Editor-RTE-Deep-Dive1.html)
...
| 機能 | 説明 |
| -----------| ---------- |  
| 検索ツール | （Google広告のアカウントを持つ広告主）Advertising Cloudは、Advertising Cloudコンバージョン追跡サービスを使用するGoogle広告キャンペーンで追跡するすべてのコンバージョンデータを、オプションでGoogle広告にアップロードできます。 毎日のアップロードには、広告主レベルのアトリビューションモデルを使用して定義されたコンバージョン値が含まれます。 アップロードされたすべての変換には「Adobe_ACS_」が先頭に付きます(変換「購読」の場合は「Adobe_ACS_購読」など)。 <br/> **注意：** アップロードには、フィードファイルから Advertising Cloud にアップロードされたコンバージョンデータは含まれません。 |
...
| 機能／拡張機能 | 説明 |
| --- | --- |
| シングルページアプリケーション Visual Experience Composer （SPA VEC） | SPA VEC に、作業を高速化または効率化するための以下の機能が追加されました。<ul><li>VECでのWebサイトの読み込みをキャンセルして、アクティビティの編集をブロック解除できるようになりました。 この強化は、例えば、アクティビティを小さく編集する場合、設定を確認する場合、またはカスタムコードを追加する場合に、サイトの読み込みを待たない場合に便利です。 （TGT-33872）</li><li>VEC でページが読み込まれる前や、ページを読み込めなかった場合（例えば、カスタムコードが動作しなくなった場合など）には、様々なアクションを実行できます。サイトが読み込まれる前に編集できない操作は、TargetUIで無効になります。 （TGT-33851およびTGT-34149）</li></ul> |
| 自動パーソナライゼーション(AP)と自動Targetアクティビティ | APまたは自動Targetアクティビティの作成時に、コントロールとして使用するエクスペリエンスを選択できます。 この機能を使用すると、アクティビティで設定されているトラフィックの配分の割合に基づいて、コントロールトラフィック全体を特定のエクスペリエンスにルーティングできます。 その後、パーソナライズされた配信のパフォーマンスをコントロールのエクスペリエンスと比較して評価できます。 （TGT-26572） |
...
Magentoは、柔軟な買い物かごシステムをオンラインマーチャントに提供し、オンラインストアの外観、コンテンツ、機能を制御するeコマースプラットフォームです。 Magentoはオープンソース版とフル機能のコマース版で入手できます。
