...
* [!DNL Experience Platform]の最新の更新については、Adobe.ioの[Adobe Experience Platformのリリースノート](https://www.adobe.io/apis/experienceplatform/home/services/release-notes.html#!end-user/markdown/release-notes/release-notes-20190515.md)を参照してください。
...
* [Analytics 管理者向けの重要な注意事項](#aa-notices)    **（更新日2019年6月27日）**
...
| **Analytics ドキュメントの改善** | Analytics ドキュメントが再構成され、コンテンツの改善を提案できる機能が追加されました。ドキュメントに関する問題を報告したり、編集を提案したりできます。ドキュメントセットは[新しいドメイン](https://docs.adobe.com/content/help/ja-JP/analytics/landing/home.html)に移動されました。 従来のサイトからリダイレクトがおこなわれます。 |
| **新しいテクニカルノートユーザーガイド** | [テクニカルノートのユーザーガイド](https://docs.adobe.com/content/help/ja-JP/analytics/technotes/home.html)が利用できるようになりました。 現在は、Google Analytics などのサードパーティ分析ツールに詳しいユーザーが Adobe Analytics に慣れることを主目的として構成されています。テクニカルノートユーザーガイドは、今後数ヶ月の間にその他のコンテンツが追加される予定です。 |
...
| 通知 | 追加日または更新日 | 説明 |
| -----------| ---------- | ---------- |
| データ収集の更新 | 追加日：2019 年 6 月 27 日 | 「same-site-by-default-cookies」フラグを持つすべてのAdobe Analyticscookieには、このフラグが`SameSite=None`に設定されるようになりました。 |
| 監査ログUIの変更 | 更新日2019年6月26日 | API `Logs.GetUsageLog` では、UI に次の変更が加えられました。 <ul><li>API の応答ヘッダーが次のように変更されました。a) `timestamp` から `dateCreated`、b) `login` から `companyLogin`、c) `event_num` から `eventType`、d) `event_type` から `event_type`、e) `ip_address` から `ipAddress`、f) `report_suite` から `rsid`、g) `event_details` から `logText`。 </li><li>タイムスタンプの形式が Unix タイムスタンプから ISO 8601 時間に変更されました。</li></ul><br>APIおよび管理ログUIで、監査ログの結果にレポートスイート名ではなく、レポートスイートIDが含まれるようになりました。 |
...
* [Cloud Manager 2019.5.0 のリリースノート](https://docs.adobe.com/content/help/ja-JP/experience-manager-cloud-manager/using/release-notes/release-notes-current.html)
...
| 監査証跡 | 管理者は、Adobe Campaign Classic インスタンスでおこなわれた変更を監視および管理することで、生産性を向上できます。監査証跡は、ソーススキーマ、ワークフローおよびオプションでおこなわれたアクションを記録します。要素が作成、変更または削除された場合、すばやく確認できます。<br>詳しくは、[詳細なドキュメント](https://docs.campaign.adobe.com/doc/AC/ja-JP/PRO_Production_procedures_Audit_trail.html)と[ハウツービデオ](https://helpx.adobe.com/jp/campaign/kt/acc/using/acc-audit-trail-feature-video-use.html)を参照してください。 |
| ガードレール、堅牢性および拡張性 |  [!DNL Campaign Classic] に一連の機能強化が加えられました。ガードレール、堅牢性、およびスケーラビリティの向上については、[Adobe Campaign Classicリリースノート](https://docs.campaign.adobe.com/doc/AC/ja/RN.html)を参照してください。 |
...
| 機能 | 説明 |
| ------------- | ----------- |
| コントロールパネル | 容量の監視やインスタンスの設定の管理（最初は SFTP サーバー管理から）が簡単にできるので、管理者ユーザーの作業効率向上を支援できます。<br>詳しくは、[詳細なドキュメント](https://helpx.adobe.com/campaign/kb/control-panel.html)と[ハウツービデオ](https://helpx.adobe.com/jp/campaign/kt/acs/using/acs-control-panel-video-use.html)を参照してください。 |
