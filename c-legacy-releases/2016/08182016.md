...
* 関数 ` Visitor.getInstance` で設定できるオプションのブール値のフラグとして、` idSyncAttachIframeOnWindowLoad` が追加されました。` idSyncAttachIframeOnWindowLoad= true` の場合、ID サービスは、ウィンドウの読み込み時に ID 同期 iFrame を読み込みます。デフォルトでは、ID サービスは、可能な限り迅速に iFrame を読み込みます。このフラグは、廃止される *に代わるものです。*` idSyncAttachIframeASAP`[ID サービスの getInstance 関数で使用される変数](https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid-variables.html)を参照してください。（MCID-122）
...
* ID サービスが訪問者 [!DNL  Marketing Cloud] ID をクライアント側またはサーバー側に生成したかどうか、または ID 呼び出しがタイムアウトしたかどうかを判断する関数が visitorAPI.js コードに追加されました。[タイムアウトトラッキング関数](https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid-timeout-functions.html)および[クライアント側訪問者 ID 生成のトラッキング](https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid-client-side-id.html)を参照してください。（MCID-116）
...
* クロスオリジンリソース共有（CORS）の DIL サポートを追加しました。[useCorsOnly](https://marketing.adobe.com/resources/help/en_US/aam/dil-use-cors-only.html) を参照してください。（AAM-25319、AAM-20139）
...
* DIL は、デフォルトで安全な HTTPS 呼び出しをおこなうようになりました。[!DNL  Audience Manager] DIL コードと visitorAPI.js コードを同じページに使用しているお客様は、DIL 変数 ` secureDataCollection= false` を設定する必要があります。[secureDataCollection](https://marketing.adobe.com/resources/help/en_US/aam/dil-secure-data-collection.html) を参照してください。
