...
SDKドキュメントについては、[Marketing Cloudソリューション用Android SDK 4.x](https://docs.adobe.com/content/help/ja-JP/mobile-services/android/overview.html)および[Marketing Cloudソリューション用iOS SDK 4.x](https://marketing.adobe.com/resources/help/ja_JP/mobile/ios/)を参照してください。 以前のリリースのリリースノートを確認するには、左側のパネルの「*以前のリリースノート*」を展開してください。
...
DIL は、Audience Manager の API データ収集コードです。7月のリリースでは、DILがバージョン6.10になります。このリリースには、ターゲットパブリッシングiFrameの呼び出しの効率化を通じてCPU使用率を削減するためのコードが改訂されています。 （MCID-419）
