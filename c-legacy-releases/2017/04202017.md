...
[Adobe Experience Cloud](https://www.adobe.com/jp/experience-cloud.html )の詳細については、Adobe Summit 2017 での発表内容を参照してください。
...
* プロファイルリンク機能を改善しました。この機能では、クラスタ内の Audience Manager UUID が 4 つある場合、1 つの [Audience Manager UUID](https://marketing.adobe.com/resources/help/ja_JP/aam/ids-in-aam.html) を他の 3 つの Audience Manager UUID へリンクさせます。（AAM-28808）
* 次の API でクエリパラメーター testSegments=true を使用して、テストセグメント ID（[SID および csegID](https://marketing.adobe.com/resources/help/ja_JP/aam/ids-in-aam.html)）を表示できるようになりました（AAM-27591）。
   * ` .../v1/segments`
   * ` .../v1/destinations/<destinationId>/mappings`

* セグメント API メソッドを Swagger に移行しました。「[Audience Manager の API コードの移行](https://marketing.adobe.com/resources/help/ja_JP/aam/c_api_swagger.html)」を参照してください。（AAM-28601）
...
[ Adobe Campaign のセキュリティ情報](https://helpx.adobe.com/security/products/campaign/apsb17-09.html) バージョン 6.11 をリリースしました。
...
<table id="table_A41D8C79DEE84AE59FE9C8D2D291FCD7"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Media Manager から Analytics へのリバースデータフィードの新しいバージョンにより、データの正確さが大幅に向上します。Analytics との統合を利用している広告主の皆様は、今後数週間のうちに新しいリバースフィードにアップグレードされます。 </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>既存の Adobe Experience Cloud オーディエンスの電子メールリストやセグメントを使用して、Google オーディエンス（検索広告のリマーケティングリスト）を作成できるようになりました。 </p> <p> 
     <ul id="ul_9F69C50A39CE45928E98F7AC1015C807"> 
      ...
      <li id="li_FA9DE45045324D5781BAF8FB113E274B">Adobe Audience Manager セグメント：（ホワイトリストに登録されている広告主専用のベータ版のみ）Campaigns Beta／Audiences／Audiences を選択して、Media Manager が宛先になっている任意の Audience Manager セグメントから Google Web サイトのタグベースオーディエンスを作成できます。これには、Adobe Experience Cloud に発行されている Adobe Analytics セグメントや、People コアサービスを使って Adobe Experience Cloud に作成されたセグメントが含まれる可能性があります。Media Manager は、Google トラッキング URL を元の Audience Manager セグメントに自動的にプッシュするので、Google 側でオーディエンスをトラックできます。 </li> 
     </ul> </p> <p>Media Manager では、Google オーディエンスの作成や編集に使用する電子メールリストや Audience Manager セグメントの顧客データは保存されません。 </p> </td> 
  </tr> 
  <tr> 
