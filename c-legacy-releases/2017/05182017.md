...
[ID 同期と一致率について](https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid-match-rates.html)に、ID サービスから ` cm.eversttech.net` に対しておこなわれる [!DNL  Adobe Advertising Cloud] 呼び出しに関する新しい情報が追加されました。
...
<table id="table_A41D8C79DEE84AE59FE9C8D2D291FCD7"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>場所または再マーケティングリストの入札調整の自動最適化オプションを有効にすると、最適化プロセスの一環として、必要に応じて、最適化されたポートフォリオ内の関連キャンペーンの関連する入札調整が、広告クラウドによって変更されるようになりました。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"></td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>（Google アカウント）Adobe Mobile コアサービスが、モバイル測定パートナー（MMP）として統合されました。広告主レベルの設定により、Adobe Mobile Services を広告主の MMP として指定できます。 </p> <p>広告主が Adobe Mobile コアサービスを使用している場合、広告がインストールされ、アプリケーション拡張設定にポストバック URL が含まれます。アプリごとに 1 度、アプリ設定のインストールイベントおよびその他のアプリ内イベントに、このポストバック URL を手動で入力します。 </p> <p>次に、Mobileコアサービスは、ポストバックURLをポーリングして、各インストールまたはその他のアプリケーション内イベントの広告クラウドデータを送信します。その後、トラッキングされたイベントはトランザクションプロパティとして使用可能になります。表示およびレポートにトランザクションプロパティを含めたり、ポートフォリオの目標を特定のトランザクションプロパティに最適化するように設定したりするには、ポートフォリオ、レポートおよび表示でトランザクションプロパティを使用できるようにする必要があります。 </p> </td> 
  </tr> 
  <tr> 
