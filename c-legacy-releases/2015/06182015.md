...
製品ドキュメントについては、[AdobeのMobile Services](https://docs.adobe.com/content/help/en/mobile-services/using/home.html)を参照してください。 以前のリリースのリリースノートを確認するには、左側のパネルの「*以前のリリースノート*」を展開してください。
...
Analytics Premiumは、[Data Workbench6.4リリース](https://docs.adobe.com/content/help/ja-JP/data-workbench/using/release-notes/release-notes.html)に次の新機能を追加しました。
...
<table id="table_FE8C9A73676345CDA450CE84111A2B72"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> 顧客レコードサービス(CRS)のエクスポート機能を使用すると、Data Workbenchデータをエクスポートして、<i>Reports &amp; Analytics</i>などの他のAnalytics機能と統合できます。 </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> ワークステーションのインストール後、「<b>サーバーへの接続を設定</b>」ワークスペースが開き、証明書のインストールとInsight.cfgファイルへのサーバー接続情報の入力に関する追加情報が表示されます。 サーバーへの接続状況の表示やプロファイルの選択もおこなえます。 </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <b>弦</b>のビジュアライゼーションを表示しているときに、セクションの上にカーソルを置いたときの表示を改善しました。 </td> 
  </tr> 
  <tr> 
...
* 処理ルールを使用してeVarの値を削除した場合、またはeVarを&#x200B;*`blank`*&#x200B;に設定した場合に、のインスタンスが増加する問題を修正しました。
...
* *`mboxPC`*&#x200B;フィールドが見つからないため、ターゲットセグメントのエクスポートに失敗していました。 修正されました。
* IDリクエストが正しくフォーマットされている。 デフォルトのPCIDの代わりに&#x200B;*`mbox3rdpartyId`* IDを使用すると、[!UICONTROL Adobe Target]が、（[!DNL  ExportIntegration.exe]を使用して）ターゲット/Data Workbench統合によって生成されたリクエストを拒否していました。 このIDリクエストは正しくフォーマットされ、スループットは成功しています。
...
* 訪問時間/サイト時間のビジネスロジックが変更されました。
   ...
   * *`visit_start_time_gmt`*&#x200B;を訪問の開始時間として使用します。 現在は、レポートの日付範囲前に開始した訪問は、否定的な影響を受けません。
   ...
   * *`visit_start_time_gmt`*&#x200B;より前のタイムスタンプに指定されたタイムスタンプ付きのヒットは、タイムデルタに絶対値が適用されるので、必ずしも&#x200B;*`Group A: <1 minute`*&#x200B;として自動的に分類されるとは限りません。
...
* 認証済み状態のサポート。 訪問者APIは、Marketing Cloud訪問者IDと共に、同じ訪問者の複数の顧客IDと顧客タイプIDを受け入れ、異なる顧客IDと&#x200B;*`authStates`*&#x200B;の範囲を区別します。
...
* Swaggerを使用したAPIドキュメントの改善を進めています。 新しいメソッドと修正されたメソッドについては、[Audience ManagerAPIドキュメント](https://bank.demdex.com/portal/swagger/index.html)の節を参照してください。
...
詳しくは、[Adobe Social](https://docs.adobe.com/content/help/ja-JP/social/using/home.html)のリリースノートを参照してください。
