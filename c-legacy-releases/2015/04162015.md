...
* と [!DNL  Marketing Cloud] の間のシングルサインオンができない問題を修正 [!DNL  Media Optimizer]しました。

**既知の問題**

* 既知の問題により、リンク [!DNL Analytics] されたアカウントによって所有されていないレポートスイートからオーディエンスを共有できません。現在、改善に取り組んでいます。
...
* ダッシュボードの日付比較設定を手動でリセットする必要があった問題を修正しました。この修正は、新しいブックマークにのみ機能します。既存のブックマークは開いて*` saved as`* する必要があります。
...
* Data Warehouseにセグメント指標としてサポートを追加して、Reports&amp; [!DNL Analytics] Data *` campaign instances`* WarehouseデータとData Warehouseデータの相違を修正。
