...
* [!DNL Analytics]アプリの初回読み込みを更新し、ログイン処理の速度を向上しました。
* [!UICONTROL フィードバックの入力]ツールを使用して *`Bug`* タイプの問題を送信すると、その問題は、直接カスタマーケアに送信され、チケットが作成されるようになりました。その後すぐにカスタマーケアから連絡いたします。
* アプリ内メッセージを作成する際に、*`Lifetime Value`*&#x200B;または&#x200B;*`Locale`*&#x200B;を使用してメッセージをターゲットできるようになりました。
...
* ダッシュボードの日付範囲の比較設定を手動でリセットする必要がある問題を修正しました。 この修正は、新しいブックマークに対してのみ機能します。既存のものは開いて&#x200B;*`saved as`*&#x200B;する必要があります。
...
* ソーシャルアプリ機能は、Adobe Social15.4.1.0（2015年4月）リリースで正式に廃止されました。 これには、すべてのアプリ関連指標、データおよびAPIメソッドが削除されます。 ご質問がある場合は、Socialのアカウントマネージャーにお問い合わせください。 詳しくは、[Adobe Socialアプリケーション — 廃止ファクトシート](https://helpx.adobe.com/social/kb/social-applications---deprecation-fact.html)を参照してください。
* [!UICONTROL 投稿[!DNL Analytics]]レポートのTwitter [!UICONTROL 合計リーチ]指標の不整合の問題を修正しました。
