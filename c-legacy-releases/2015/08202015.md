...
* 分類ルールビルダーの[レポートスイートと変数を選択](https://docs.adobe.com/content/help/en/analytics/components/classifications/classifications-rulebuilder/classification-rule-definitions.html)パネルが更新されました。このパネルでは、レポートスイートと変数を既存のルールセットに追加できます。以前は、ルールセットの作成中に変数とレポートスイートを指定していました。この変更により、分類変数での発生が報告されていた問題が起きなくなります。
...
* [!UICONTROL 訪問者 ID サービス]で、マルチパートのトップレベルドメイン cookie を適切に設定できないバグを修正しました。例えば、` my_company.co.uk` のようなドメインの場合、状況によっては、[!UICONTROL 訪問者 ID サービス]は、` co.uk` にのみ cookie を設定していました。（AN-104683）
   ...
   * 訪問者 ID サービスを使用している。
   * [猶予期限](https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid_grace_period.html)*を有効にしているか、*猶予期限がファーストパーティ cookie を使用していて、ユーザーが cookie をブロックしている。
...
* 新しいドキュメントで、Google Doubleclick Campaign Manager（DCM）アカウントから [!DNL  Audience Manager] にデータを移行する方法を説明しています。[Audience Manager への DCM ファイルの読み込み](https://marketing.adobe.com/resources/help/en_US/aam/t_dcmlogs.html)を参照してください。
...
* [Master Marketing Profile との統合](https://marketing.adobe.com/resources/help/en_US/aam/c_mmp.html)が改訂され、コンテキストデータ変数が更新されました。この変数は、アンダースコアで終わる必要があります。以前は、ピリオドで終わっていました。

**既知の問題**

[!DNL  Analytics] のお客様は、[Master Marketing Profile](https://marketing.adobe.com/resources/help/en_US/aam/c_mmp.html) を使用し、[!DNL  Audience Manager]訪問者 ID サービス[を使用して宣言済み ID を ](https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid_service.html) に渡す場合、統合コードを設定する必要があります。詳しくは、[プロファイルの結合を有効にする](https://marketing.adobe.com/resources/help/en_US/aam/c_profile_merge_enablement.html)を参照してください。
