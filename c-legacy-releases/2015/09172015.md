...
* [キャンペーン](../../c-legacy-releases/2015/09172015.md#campaign)
...
* [Marketing Cloud インターフェイス](../../c-legacy-releases/2015/09172015.md#marketing_cloud_interface)
...
* 顧客属性データをアップロードする際に、断続的なタイムアウトが発生するパフォーマンスの問題を修正しました。（MAC-26305）
...
Mobile Services の 2015 年 9 月 18 日リリースには、次の変更が含まれています。
  ...
  <tr> 
   ...
   <td colname="col2"> <p>ユーザーが Apple App Store や Google Play ストアからアプリを直接ダウンロードするためのアプリストアリンクまたは QR コードを作成します。作成したリンクは、ダウンロードおよびその他の成功イベントを獲得キャンペーンの成果にできます。 </p> <p>以下の点が強化されました。 </p> 
    <ul id="ul_49F482D158CE47769A31164318B49BAD"> 
     ...
     <li id="li_926E799E93EE41B2A5A5075CEE07F941">URL の末尾に、アフィリエイト ID などのデータを追加できるようになりました。URL にデータを追加すると、コンテキストデータとして取得できるので、保存したりレポートに使用したりできます。 </li> 
...
**Marketing Cloud の訪問者 ID サービスツールのアップデート：** DTM は、9 月 23 日に Marketing Cloud の訪問者 ID サービスツールのアップデートをリリースします。最新のライブラリバージョン（1.6.1）では、顧客 ID のサポートが強化され、認証状態のサポートが追加されます。また、マルチソリューションの統合も強化されます。
...
Dynamic Tag Management の 2015 年 9 月 4 日リリースには、次の変更が含まれています。
...
>[!IMPORTANT]
>
>2016 年 1 月に、アドビは、セキュリティ上の理由から、SSL v3 プロトコルのサポートを無効にし、Adobe Analytics データ収集用の SSL v3 トラフィックを終了します。このアクションによって多大な影響を受けるお客様には、別途連絡差し上げます。
...
Analytics の 2015 年 9 月 18 日リリースには、次の変更が含まれています。
...
* Reports &amp; Analytics でユーザーグラフを作成した後、デフォルトのユーザーグループとして選択すると発生していた問題を修正しました。デフォルトのグループを「_「なし_ 」はエラーメッセージで失敗しました。（AN-108270）
* 訪問者プロファイルおよびampを使用できない問題を修正しました。gt;顧客属性レポートがメニューに表示されます。（AN-106265）
...
** [!DNL  AppMeasurement] for JavaScript** JavaScript 版 AppMeasurement
...
<table id="table_CE3C48DBA4A343B3B694ACAB96E11279"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> <span class="wintitle">アドレス可能なオーディエンス</span>には、セグメント間の重複（マッチ率）および過去 60 日間の送信先が表示されます。データのビジュアライゼーションおよび数値指標とともに結果を表示します。重複データは、送信先へのセグメントの送信時に、Destination の「基本情報」セクションに表示されます。<span class="wintitle"> アドレス可能なオーディエンス</span>は自動的に使用可能になり、Destination がサーバー間通信の場合のみ返します。 </p> <p> <b>注意：</b>リリース時（9 月 30 日）には、<span class="wintitle">アドレス可能なオーディエンス</span>は次のようになります。 
