...
| 機能 | 説明 |
| ------- | ----------- |
| 広告インサイト | （9月8日）ベータモードでは、追加のインサイトを利用できます。 |  | ブランドセーフティ | （9月22日リリース）[!DNL DoubleVerify] ブランドセーフティプレビッド製品が [!DNL Brand Suitability Tiers] に更新されました。これにより、広告主は特定のトピックのすべてのインスタンスを避けることなく、特定のセグメントに対して 3 つのリスクレベル（低、中、高）から選択できるようになりました。これまでは、セグメントに許容値のレベルが含まれていませんでした。 |
