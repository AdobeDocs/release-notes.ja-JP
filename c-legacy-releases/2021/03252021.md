...
| プライベート在庫 | （10 月 15 日（PT）リリース）すべてのユーザーが、新しい Deal ID フォームを使用して、Deal ID の詳細を設定および編集できるようになりました。これは、従来の [!UICONTROL Smart Ad Serving] フォームをシンプルにしたバージョンです。新しい取引 ID の詳細を設定するには、**[!UICONTROL 在庫／取引]**&#x200B;に移動し、「**[!UICONTROL 作成]**」を選択してから、「**[!UICONTROL 取引 ID ベータ版]**」をクリックします。 |
