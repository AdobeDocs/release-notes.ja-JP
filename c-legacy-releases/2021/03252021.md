...
| [顧客のアクティビティハブブループリント](https://experienceleague.adobe.com/docs/blueprints-learn/architecture/audience-activation/customer-activity.html) | 外部アプリケーションが Adobe Experience Platform の[!UICONTROL リアルタイム顧客プロファイル]にアクセスする方法を説明します。 |
...
| [プロファイルエンリッチメント向けカスタムデータサイエンスのブループリント](https://experienceleague.adobe.com/docs/blueprints-learn/architecture/data-exploration/data-science.html) | [!UICONTROL Data Science Workspace] で Adobe Experience Platform のデータを使用してモデルをトレーニング、デプロイ、スコアリングし、機械学習のインサイトを提供するしくみについて説明します。 |
| [データの準備と取り込みのブループリント](https://experienceleague.adobe.com/docs/blueprints-learn/architecture/data-ingestion/overview.html?lang=ja) | ソースデータを[!UICONTROL エクスペリエンスデータモデル] (XDM) スキーマにマッピングする方法を説明します。 このブループリントには、日付の形式設定、フィールドの分割、連結、変換などのデータ変換の実行や、レコードの結合、結合、キー変更の実行も含まれます。 |
| [企業データの調査およびレポートのブループリント](https://experienceleague.adobe.com/docs/blueprints-learn/architecture/data-exploration/overview.html) | Experience Platform の[!UICONTROL クエリサービス]では、データに対して SQL クエリを実行できます。 [!UICONTROL Data Science Workspace] を使用して、データ探索、データサイエンス、および機械学習のワークロードをデータに対して実行する方法について説明します。 |
...
| [サーバー側のエンタープライズデータ収集ブループリント](https://experienceleague.adobe.com/docs/blueprints-learn/architecture/data-ingestion/server-side-collection.html) | Adobe Experience Platform Web およびモバイル SDK で収集されたデータを、Experience Platform [!UICONTROL Edge Network] から目的の宛先に転送する方法を説明します。 |
...
| 機能 | 説明 |
| -----------| ---------- |
| 統合検索 | 統合検索(Experience Platformで使用可能)で、Experience Platformユーザーのソースと宛先の検索がサポートされるようになりました。 この機能を使用すると、セグメント、データセット、スキーマ、ソース、宛先を検索できます。 |
...
| 2021 年 3 月 | [ソースコネクタを使用したデータのストリーミング](https://experienceleague.adobe.com/docs/platform-learn/tutorials/sources/streaming-ingestion-source-connector.html?lang=ja) | ビデオ | このビデオでは、クラウドストレージソースからPlatformにリアルタイムでデータをストリーミングし、顧客エンゲージメントにリアルタイムでデータを使用する方法を説明します。 |
...
| 2021 年 3 月 5 日（PT） | [API を介したストリーミングデータ取得](https://experienceleague.adobe.com/docs/platform-learn/tutorials/sources/streaming-ingestion-http-api.html?lang=ja) | ビデオ | このビデオでは、HTTP APIエンドポイントを使用して、Adobe Experience Platformにリアルタイムでデータをストリーミングする方法を説明します。 |
...
| 2021 年 3 月 5 日（PT） | [Amazon S3 からのデータ取得](https://experienceleague.adobe.com/docs/platform-learn/tutorials/sources/ingest-data-from-cloud-storage.html) | ビデオ | このビデオでは、クラウドストレージサービスから Adobe Experience Platform のリアルタイム顧客プロファイルとデータレイクにデータをシームレスかつスケーラブルに簡単にバッチ取得する方法を説明します。 |
| 2021 年 3 月 5 日（PT） | [Salesforce CRM からのデータ取得](https://experienceleague.adobe.com/docs/platform-learn/tutorials/sources/ingest-data-from-crm.html) | ビデオ | このビデオでは、CRM ソースから Adobe Experience Platform のリアルタイム顧客プロファイルとデータレイクにデータをシームレスかつスケーラブルに簡単にバッチ取得する方法を示します。 |
| 2021 年 3 月 5 日（PT） | [Adobe Analytics からのデータ取得](https://experienceleague.adobe.com/docs/platform-learn/tutorials/sources/ingest-data-from-adobe-analytics.html) | ビデオ | Adobe Analytics ソースコネクタを使用すると、Adobe Analytics から Adobe Experience Platform のリアルタイム顧客プロファイルと Experience データレイクにデータをシームレスかつスケーラブルに簡単にストリーミングできます。 |
...
| 2021 年 3 月 5 日（PT） |   [Adobe IO コンソールの Postman 詳細エクスポート](https://experienceleague.adobe.com/docs/platform-learn/tutorials/platform-api-authentication.html) | ビデオ | Experience Platform API を認証しアクセスする方法を説明します。 |
...
* [Analytics管理者向けの重要な注意事項](#aa-notices) **（2021年4月7日更新）**
* [AppMeasurement](#appm)
...
* [Experience Manager 6.4 累積修正パックリリースノート](https://experienceleague.adobe.com/docs/experience-manager-64/release-notes/cfp-release-notes.html?lang=en)
...
| 2021 年 3 月 | [AEM パブリッシュ環境を使用した実稼働デプロイメント](https://experienceleague.adobe.com/docs/experience-manager-learn/getting-started-with-aem-headless/graphql/multi-step/production-deployment.html) | チュートリアル | オーサーインスタンスからパブリッシュインスタンスに配布されるコンテンツをシミュレートするローカル環境を設定します。 GraphQL API を使用して AEM パブリッシュ環境のコンテンツを使用するように設定された React アプリの実稼働版ビルドを生成します。 この過程で、環境変数を効果的に使用する方法と、AEM CORS 設定を更新する方法について学びます。 |
...
| 2021 年 3 月 | [基盤 - ワークフロー管理](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/sites/administering/workflows-administering.html?lang=en) | ビデオ | このビデオでは、ワークフローモデルを使用して、この機能セットを紹介します。ただし、これらのワークフローモデルは AEM ランチャーにも当てはまります。 |
...
* [Experience Manager ドキュメントの以前のバージョン](https://docs.adobe.com/content/help/en/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions.html)
