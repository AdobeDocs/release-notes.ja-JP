...
* [Analytics](#analytics) および [Customer Journey Analytics](#cust-journey) ( リリース日： **2021 年 1 月 15 日**)
...
サポートが必要な場合[ Experience League](https://experienceleague.adobe.com/?lang=ja#home) にアクセスし、製品および技術ドキュメント、アドビが運営するコース、ビデオチュートリアル、すばやい回答、コミュニティインサイト、講師によるトレーニングをお探しください。コンテンツは`experienceleague.adobe.com`からこの場所に移動されました。 状況に応じてブックマークを更新してください。
...
Experience Cloudサービスおよび管理ドキュメントには、顧客属性、オーディエンスライブラリ ([!UICONTROL 人] サービス )、アクティベーション、ユーザーと製品の管理、Experience Cloudの cookie。
...
一般 — SDK は、 [!DNL XCFrameworks] 既存のApple®アーキテクチャのサポートを維持しながら、新しいインテル M1 アーキテクチャでハードウェアをサポートするため。
...
| 2020 年 12 月 9 日（PT） | [Offer Decisioning でのルールの作成](https://experienceleague.adobe.com/docs/offer-decisioning-learn/tutorials/create-rules.html?lang=ja) | ビデオ | [!UICONTROL Offer Decisioning] でルールを作成する方法を説明します。 ルールは、Platform の [!UICONTROL リアルタイム顧客プロファイル] およびを、オファーの実施要件制約から削除する場合に使用します。 |
...
| Adobe Data Connectors のサポート終了 | 2020 年 7 月 13 日（PT） | Adobe [!UICONTROL Data Connectors] は、実行不可能またはサポート対象外のレガシーテクノロジーによって動作します。[Adobe Exchange パートナープログラム](https://partners.adobe.com/exchangeprogram/experiencecloud)には、引き続き提供およびサポートを希望する統合に対して採用する新しい基準があります。正式な終了日はまだ決定していませんが、今後 12 ～ 18 か月（2021年半ば ～ 2021年末）の間になると予想されています。[詳細情報...](https://experienceleague.adobe.com/docs/analytics/import/dataconnectors/data-connectors-eol.html?lang=ja) |
...
Experience Manager6.5、Service Pack 7（2020 年 11 月 27 日にリリースされた 6.5.7.0）は重要なアップデートであり、2019 年 4 月のExperience Manager6.5 の一般リリース以降にリリースされた新機能、主なお客様向けの機能強化、パフォーマンス、安定性およびセキュリティが含まれています。
...
Experience Manager6.4、Service Pack 8、累積 Fix Pack 3（2020 年 11 月 27 日にリリースされた 6.4.8.3）は重要なアップデートであり、2020 年 3 月のExperience Manager6.4、Service Pack 8(6.4.8.0) の一般リリース以降の内部およびお客様向けの修正が含まれています。
...
詳しくは、[CIF コアコンポーネント](https://github.com/adobe/aem-core-cif-components/releases/tag/core-cif-components-reactor-1.6.0)を参照してください。
   * **Cloud Manager**
      ...
      * のセルフサービス管理 [SSL 証明書](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/using-cloud-manager/manage-ssl-certificates/introduction.html?lang=ja) および [カスタムドメイン名](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/using-cloud-manager/custom-domain-names/introduction.html?lang=ja).
      * のセルフサービス管理 [IP許可リスト](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/using-cloud-manager/ip-allow-lists/introduction.html?lang=ja).
      * 環境の詳細ページが更新され、環境における[!UICONTROL カスタムドメイン名]と [!UICONTROL IP 許可リスト]を管理できるようになりました。
   * **コードリファクタリングツール**

      * AIO-CLI プラグインの新しいバージョンがリリースされました。 このプラグインの最新バージョンには、Experience ManagerDispatcher Converter と Repository Modenizer のバグ修正が含まれ、新しいユーティリティであるインデックスコンバーターもサポートされています。
このプラグインの詳細については、[統合エクスペリエンス](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/moving/refactoring-tools/unified-experience.html?lang=ja#benefits)を参照してください。
      * インデックスコンバーターは、お客様のカスタム OAK インデックス定義を、as a Cloud Serviceの互換性のある OAK インデックス定義にExperience Manager変換するために使用できるユーティリティです。
...
* **ベストプラクティスアナライザー**
   ...
   * [!UICONTROL Cloud Readiness アナライザー]は、[!UICONTROL ベストプラクティスアナライザー]（BPA）に変更されました。BPA は、現在のExperience Manager実装のベストプラクティスの評価を提供し、既存のAEMインスタンスからAEM as a への移行に対する準備状況を評価するのに役立ちます。 [!UICONTROL Cloud Service].
...
| 2020 年 12 月 9 日（PT） | [API の互換性](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/assets/admin/developer-reference-material-apis.html?lang=ja) | 記事 | 様々な用途で使用できるAEM API(npm、Java™、HTTP) を明確に伝える簡単な記事を作成します。 [!UICONTROL Assets] 操作。 |
...
| 機能 | 説明 |
| -----------| ---------- |
| 新しいヘルプ | （10 月 28 日のリリース）従来のヘルプは更新されたページに置き換えられました。ページはDSPのメインメニューの「ヘルプ」リンクから利用でき、常に次の場所から利用できます。 [https://experienceleague.adobe.com/docs/advertising-cloud/dsp/home.html](https://experienceleague.adobe.com/docs/advertising-cloud/dsp/home.html?lang=ja) |
...
| 2020 年 12 月 29 日（PT） | [入力可能なフォームの作成](https://experienceleague.adobe.com/docs/document-cloud-learn/acrobat-learning/getting-started/create-fillable-forms.html?lang=ja) | 記事 | InDesign、Microsoft® Word や Excel で作成したスキャン済みの紙のフォームやドキュメントを、入力可能なPDFフォームに変換します。 |
...
| 2020 年 12 月 28 日（PT） | [フォームデータの使用](https://experienceleague.adobe.com/docs/document-cloud-learn/acrobat-learning/advanced-tasks/formdata.html?lang=ja) | 記事 | 完成した一連のフォームがあり、データをコンパイルする必要がある場合、Acrobat DCを使用して、回答を単一のスプレッドシートに結合できます。 |
