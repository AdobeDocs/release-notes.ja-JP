...
* 一部のマーケティングチャネルインスタンスが「なし」と表示される問題を修正しました。 （AN-257850）
* Windows および Chrome のオペレーティングシステムが、オペレーティングシステムの種類ディメンションで「その他」の下にグループ化されていた問題を修正しました。 （AN-244234）
...
| データフィードと Data Warehouse の IP アドレスの変更 | 2021 年 4 月 6 日（PT） | 6 月 17 日（PT）より、アドビのデータセンター内でデータフィードおよび Data Warehouse 配信システムの再配置が行われます。そのため、表示される外部 IP アドレスが変更される可能性があります。レポートおよびフィードが提供されるデータセンターのすべての IP CIDR ブロックが、制御する宛先システムのファイアウォール内に存在することを確認しておくことを推奨します。 [ファイアウォールのに配置する IP アドレス範囲の完全なリストは、次のとおりで許可リストす。](https://experienceleague.adobe.com/docs/analytics/technotes/ip-addresses.html?lang=ja#data-collection-and-ftp-ip-address-blocks). |
...
| 2021 年 5 月 | [データビューでのコンポーネント設定の指定](https://experienceleague.adobe.com/docs/customer-journey-analytics-learn/tutorials/configuring-component-settings-in-data-views.html?lang=ja) | ビデオ | を設定する際に、各コンポーネント（指標およびディメンション）に対して設定する基本的なコンポーネント設定について説明します。 [!UICONTROL データビュー] Customer Journey Analytics |
...
* **Adobe Experience Manager as a Cloud Service**
   ...
   * **Experience Manager Forms as a Cloud Service**
      ...
      * **Adobe Sign 対応アダプティブフォームでの政府機関 ID 検証方法の使用**
         ...
         高度な機械学習アルゴリズムを活用したAdobe Signの政府 ID プロセスにより、世界中の企業が受信者の ID を高品質に検証できるようになります。 Adobe Sign 対応のアダプティブフォームで、政府機関 ID の ID 検証方法を使用できるようになりました。

         政府機関 ID は、質の高い ID 検証方法です。受信者に [政府発行の ID 文書（運転免許証、国籍 ID、パスポート）の画像をアップロードする](https://helpx.adobe.com/jp/sign/using/adobesign-authentication-government-id.html). 次に、そのドキュメントが真正であるかどうかが確認されます。
...
| 機能 | 説明 |
| -----------| ---------- |
| パッケージ設定 | 新しい[!UICONTROL ペーシングフィル戦略]オプション「[!UICONTROL Slightly Ahead]」が利用可能で、新しいパッケージのデフォルトとなっています。この方法では配信が加速され、飛行時間の途中で 55～65%完了するようになります。 |
...
| 公開日 | 名前 | タイプ | 説明 |
| -----------| ---------- | ---------- | ---------- |
| 2021 年 5 月 | [PDF オンラインエクスペリエンスの制御と分析の収集](https://experienceleague.adobe.com/docs/document-services/tutorials/pdfembed/controlpdfexperience.html?lang=ja#part1) | 記事 | Adobe PDF Embed API を使用して、外観を制御し、コラボレーションを可能にし、PDFとのやり取りに関する分析を収集する方法について説明します。ページでの滞在時間や検索に費やした時間などが含まれます。 （4 部） |
