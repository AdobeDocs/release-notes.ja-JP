...
* [Adobe Experience Manager 6.5 Service Pack リリースノート](https://experienceleague.adobe.com/docs/experience-manager-65/release-notes/service-pack/sp-release-notes.html?lang=en)
...
最終更新日： **6 月 26 日リリース向け**

| 機能 | 説明 |
| ------- | ----------- |
| [!UICONTROL Campaigns] | (Google Ads キャンペーン ) [!UICONTROL Advertising Cloud Search] 買い物キャンペーンでローカルの在庫広告のアトリビューションを同期、最適化、提供するようになりました。<br><br>Google Ads 内でローカルの在庫広告を設定し、管理します。 |
|  | 複数のキャンペーンおよびキャンペーンコンポーネントを編集する際に、変更をドラフトとして保存して保留中ビューでレビューすることや、特定の時間に変更をスケジュールすることができなくなりました。 既存のスケジュール済みジョブは、すべてスケジュール済みとして実行されます。 |
| [!UICONTROL 広告インサイト] | ベータモードでは、次のインサイトを利用できます。<ul><li>[!UICONTROL 正規化された Sim（結合）ベータ]</li><li>[!UICONTROL Portfolio起動ベータ版]</li> |
| [!UICONTROL 通知センターベータ版] | ブラウザー外で、 [!UICONTROL 通知センターベータ版] Google Chrome 40 以降またはMicrosoft Edge 17 以降用のアプリケーション。 アプリケーションは、通常のブラウザーバージョンよりも高速に読み込まれ、自動的に更新されます。<br><br>アプリケーションをインストールすると、Web アプリケーションは、別のウィンドウとして読み込まれ、ウィンドウサイズに基づいてレイアウトが動的に再配置されます。 アプリケーションにログインする必要はなく、使用するためにブラウザーウィンドウを開く必要もありません。 |
|  | 新しいアラートタイプ [!UICONTROL Campaign Management UI アクション]を使用でき、ポートフォリオ、制約、分類、イベントへの一括編集や割り当て変更を含む、すべてのバックグラウンドジョブの最終ステータスを表示します。 UI アクションのアラートを自動的に購読しました。<br><br>このアラートタイプは、最終的に、右側のツールバー（従来のジョブステータスの通知）から使用できる従来のジョブステータスの通知を置き換えます。 |
| レポート | 新しい [!UICONTROL キャンペーン時間別レポート] 指定した日付範囲の 1 時間ごとのキーワードを持つ 1 つ以上のGoogleキャンペーンのコスト、クリック、（オプションで）コンバージョンデータを表示します。 このレポートは [!UICONTROL キャンペーンレポート] (「時間別」または「曜日（時間別）」のデータ集計を使用 ) これらの集計タイプは、 [!UICONTROL キャンペーンレポート] 今後のリリースで使用できるようになります。 |
