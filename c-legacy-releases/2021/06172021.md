...
|  | 新しいアラートタイプ [!UICONTROL キャンペーン管理 UI のアクション]を使用でき、ポートフォリオ、制約、分類、イベントへの一括編集や割り当て変更を含む、すべてのバックグラウンドジョブの最終ステータスを表示します。 UI アクションのアラートを自動的に購読しました。<br><br>このアラートタイプは、最終的に、右側のツールバー（従来のジョブステータスの通知）から使用できる従来のジョブステータスの通知を置き換えます。 |
