...
<table id="table_91D1FD20C4C1411292252364328677AF"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>以前は、ディメンションが列にドロップされた場合、非時間ディメンションには上位5個の値（および時間ディメンションには15個）を表示し、それらの値を静的に保っていました（つまり、選択された5個の値が変更されませんでした）。 </p> <p>今後は、デフォルトでは静的な値ではなく動的な値が表示されるとともに、静的な値に変更するオプションも用意されます。その他の注意事項： </p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> Analyticsの使用状況ログで、誰かが<span class="uicontrol">Analysis Workspace</span>でプロジェクトを開いた日時を追跡するようになりました（作成、更新、共有解除、削除の既存の追跡に加えて）。 これにより、誰がどれくらいの頻度でワークスペースを使用しているのかを調べられるようになりました。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col01"> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> セグメントまたはアラートを定義する際に、セグメントまたはアラートの一部のコンポーネント（指標/ディメンション/セグメント）に現在選択されているレポートスイートとの互換性がない場合は、「<b>互換性のないコンポーネント」メッセージ</b>が表示されます。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_A6749BA62D5B40479B949EA1C64E4B7F"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>2018 年 6 月、アドビは TLS 1.0 プロトコルのサポートを終了します。Adobe<span class="wintitle">Audience Manager</span>の機能は、TLS 1.0のみをサポートするブラウザを使用するエンドユーザーには動作しません。ブラウザ、バージョン、TLSのサポートのリストを収集しました。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_A41D8C79DEE84AE59FE9C8D2D291FCD7"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>共有AnalyticsセグメントからGoogle AdWordsリマーケティングリスト(RLSA)を作成できるようになりました。 以前は、広告主もAdobe<span class="wintitle">Audience Manager</span>を持つ必要がありました。 資格を得るには、広告主はAdobe Experience CloudIDサービスを実装し、IMS組織IDを提供し、Webサイトにタグをデプロイする必要があります。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>一括送信シートの検索 </p> </td> 
   <td colname="col2"> <p>（Google AdWordsアカウント）一括送信シートで、キャンペーンのリマーケティングリスト(RLSA)ターゲットがサポートされるようになりました。 </p> <p>AdWordsキャンペーン用の一括送信ダイアログには、「キャンペーンRLSAターゲット」のオプションのエンティティセクションが含まれ、「キャンペーン名」、「入札調整」、「広告グループ名」、「オーディエンス」、「ターゲットタイプ」（値<i>inclusion</i>）、「RLSAターゲットステータス」および「Adobe」推奨入札調整」 </p> </td> 
  </tr> 
  <tr> 
