...
管理者ユーザーがサブドメインをデリゲートし、Campaign コントロールパネルから SSL 証明書を更新する新しい機能が追加されました。
...
| Search／Campaigns、Reports | Google Ads の目立ち度の指標「Impr. (Abs.Top) %」および「Impr. 「（上位） %」が、買い物商品グループ、キャンペーン日別インプレッション共有レポート、キーワード日別インプレッション共有レポート、ラベルと制約ビューを除く、すべての基本レポートとエンティティレベルのキャンペーン管理ビューで使用できるようになりました。 |
