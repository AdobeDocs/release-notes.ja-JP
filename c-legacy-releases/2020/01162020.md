...
* Adobe ID を使用し、製品、地域およびイベントの環境設定に基づいてイベント通知を購読できます。購読の環境設定をおこなうユーザーは、関連する製品のインシデントとメンテナンスイベントについてのみ、それらが開かれたり、更新されたり、クローズされたりしたときに通知されます。 はじめに [status.adobe.com/subscriptions](https://status.adobe.com/proactive-notifications/subscriptions/edit) をご覧ください。
...
Adobeは、Chrome 80（2020 年 2 月にリリース予定）で Chrome がおこなう変更に備えて、cookie に関する `same-site` 設定を調整しています。
