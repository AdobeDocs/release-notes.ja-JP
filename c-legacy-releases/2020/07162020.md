...
| 機能 | 説明 |
| -----------| ---------- |
| GDPR透明性と同意の枠組み | Advertising Cloudは、2020年8月15日までに、IABの透明性と同意の枠組みのサポートをバージョン2.0にアップグレードしています。 現時点では、お客様側で変更を行う必要はありません。  各お客様のデータ・プロセッサとして、Adobe Advertising Cloudは、GDPRに基づくお客様の義務を満たすための機能を備えており、当社の製品やサービスの使用に適用されます。 |
| Audience Manager統合 | (Adobe Experience Platformをお持ちのAdobe Audience Managerのお客様のベータ版のみ)DSPは、広告主の広告関与データを含む実用的なログファイルを、Adobe Experience Platform経由でAudience Managerに送信できます。 Audience Managerでは、広告主は、Audience Optimizationレポートなどのインサイトツールで、データをアドバンスセグメント化や使用に使用できます。 |
| 人物ベースのクロスデバイスグラフのターゲット設定 | セルフサービスのDSPのお客様は、新しいキャンペーンにわたる人ベースのターゲティングと頻度管理に、Adobeグラフ（Device Device Co-opまたはLiveRamp）を使用できます。 これにより、広告主は所有するデバイス全体でオーディエンスに到達できますが、公開数を制限できます。 さらに、複数のプレースメントにわたるデバイス間のターゲット設定のバルク編集がベータ版になりました。 お使いのアカウントでベータ版を有効にするには、Adobeのアカウントマネージャーにお問い合わせください。 |
| サイトのターゲット設定 | 「ブラックリスト」と「ホワイトリスト」は、より公平な用語「ブロック（サイト）」と「ターゲット（サイト）」に変更されました。 |
| flashtalking広告 | DSPは、すべてのflashtalking広告タグに対して、単一の広告作成と一括サードパーティ広告作成ワークフローを使用して、自動的にクリックマクロを挿入するようになりました。 |
| レポート | (Adobe Experience Platformのお客様専用)Advertising Cloud Searchの新しいクロスチャネルレポートは、DSPの配置と検索キャンペーンに関する日々の主要指標と共に、DSPと検索でのメディア支出の統合表示を提供します。 |
