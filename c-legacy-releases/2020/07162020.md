...
| 機能 | 説明 |
| -----------| ---------- |
| GDPR の透明性と同意に関するフレームワーク | Advertising Cloudは、2020 年 8 月 15 日までに IAB の Transparency and Consent Framework のサポートをバージョン 2.0 にアップグレードします。 現時点では、お客様側で変更は必要ありません。  Adobe Advertising Cloudは、各お客様のデータ処理者として、お客様が GDPR に基づく義務を果たすために利用できる機能を備えており、アドビの製品およびサービスの使用に適用されます。 |
| Audience Managerの統合 | (Adobe Experience Platformを使用しているAdobe Audience Managerのお客様のみ )DSPは、広告主の広告エンゲージメントデータを含む実用的なログファイルを、Adobe Experience Platformを通じてAudience Managerに送信できます。 Audience Managerでは、広告主は、データをAudience Optimizationレポートなどのインサイトツールで高度なセグメント化や使用に使用できます。 |
| People-Based Cross Device Graph Targeting | セルフサービスDSPのお客様は、新しいキャンペーンをまたいだユーザーベースのターゲティングと頻度管理に、Adobeグラフ（Device Co-op または LiveRamp）を使用できます。 これにより、広告主は所有するデバイス間でオーディエンスに到達できますが、公開を制限できます。 さらに、配置全体でのクロスデバイスターゲティング設定の一括編集がベータ版になりました。 お使いのアカウントでベータ版を有効にするには、担当のAdobeアカウントマネージャーにお問い合わせください。 |
| サイトのターゲット設定 | 「ブラックリスト」と「ホワイトリスト」は、より公平な用語「ブロック（サイト）」と「ターゲット（サイト）」に変更されました。 |
| Flashtalking 広告 | DSPは、単一の広告作成ワークフローと一括のサードパーティ広告作成ワークフローを使用して、すべての Flashtalking 広告タグのクリックマクロを自動的に挿入するようになりました。 |
