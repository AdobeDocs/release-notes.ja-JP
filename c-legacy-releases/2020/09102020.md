...
| 2020 年 8 月 18 日 | [Google カスタマーマッチ](https://experienceleague.adobe.com/docs/platform-learn/tutorials/rtcdp/integrate-with-google-customer-match.html?lang=ja) | ビデオ | このビデオでは、AdobeのReal-Time CDPとGoogleのカスタマーマッチ機能が、企業の目標と共に、Googleが所有および運用するプロパティを顧客との関わりを高める方法を示します。 |
...
* を [!UICONTROL Workspace] 列を「未指定」ディメンション別に表示します。 （AN-222393）
...
| 機能 | 説明 |
| -----------| ---------- |
| VAST インベントリを含むように拡張されたインタラクティブなプリロール | 各インタラクティブプリロール配置と広告で、VPAID と VAST の両方のインベントリをサポートできるようになりました。**注意：** 主な KPI が視聴性である場合は、VPAID と VAST の配置と広告を引き続き個別に作成します。これは、VAST 広告に表示可能なインプレッションを使用できないためです。 |
