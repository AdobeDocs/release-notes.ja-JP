...
| 2020 年 8 月 18 日 | [Google カスタマーマッチ](https://experienceleague.adobe.com/docs/platform-learn/tutorials/rtcdp/integrate-with-google-customer-match.html?lang=ja) | ビデオ | このビデオでは、AdobeのReal-Time CDPとGoogleのカスタマーマッチ機能が、ビジネス目標を持つGoogleの所有および運営施設でブランドが顧客とのエンゲージメントを図り、アウトリーチキャンペーンを増やすのに役立つ仕組みを説明します。 |
...
| [!UICONTROL クロスデバイス分析]でのフィールドベースのステッチ（世界中で利用可能） | 2020 年 9 月 1 日 | 新しい [!UICONTROL &#x200B; クロスデバイス分析 &#x200B;] のお客様は、このシンプル化された実装により、デバイスグラフ（Co-op またはプライベート）を使用する代わりに、Analytics フィールド（prop またはeVar）に保存されたユーザー ID に基づいてステッチできます。 この機能強化により、ECID を実装する要件が削除され、CDA のために ID 同期を実装する要件が削除されました。 （他の特定の機能を使用するには、ECID と ID の同期が必要です）。 |
...
| 2020 年 8 月 14 日 | [&#x200B; セグメントの構築時のコードビューの使用 &#x200B;](https://experienceleague.adobe.com/docs/audience-manager-learn/tutorials/build-and-manage-audiences/traits-and-segments/using-code-view-when-building-segments.html?lang=ja) | ビデオ | コードビューを使用してセグメントを定義し、最新性や頻度の使用を含む複雑な特性の組み合わせを作成する方法について説明します。 |
...
* **AEM6.5.6.0**
6.5.6.0 AEM 6.5、Service Pack 6 （2020 年 9 月 3 日（PT）にリリース）は、2019 年 4 月のAEM 6.5 の一般リリース以降にリリースされた新機能、お客様向けの主な機能強化、パフォーマンス、安定性、セキュリティの向上を含む重要なアップデートです。
...
* **AEM6.4.8.2**
AEM 6.4、サービスパック 8、累積修正パック 2 （2020 年 9 月 3 日（PT） 6.4.8.2 リリース）は、2020 年 3 月のAEM 6.4 サービスパック 8 （6.4.8.0）の一般リリース以降に対応された、内部およびお客様向けの修正を含む重要な更新です。
...
| 機能 | 説明 |
| -----------| ---------- |
| VAST インベントリを含むように拡張されたインタラクティブなプリロール | 各インタラクティブプリロール配置と広告で、VPAID と VAST の両方のインベントリをサポートできるようになりました。**メモ：** メイン KPI がビュー機能の場合は、ビューアインプレッションは VAST 広告では使用できないので、VPAID プレースメントと VAST プレースメントと広告を引き続き個別に作成します。 |
