...
| Privacy Reporting: Analytics Admin Console | Enabling Privacy Reporting for Analytics adds a set of reserved variables to a report suite.  これらの変数は、ヒットレベルでの消費者の同意データの収集を支援するように設計されています。<br/>新しいディメンション：<br/><ul><li>Consent Management Opt-Out</li><li>同意管理オプトイン</li><li>同意管理変数： `[Link to new Consent Variables page in Analytics]()`</li></ul> |
...
| Analysis Workspace: Option to remove Unspecified/None | 「未指定（なし）」を簡単に削除する機能が、フィルターをレポートするオプションとして追加されました。 |
