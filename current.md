...
* ページでの滞在時間に影響していた問題を修正しました。Workspace レポートでは、滞在時間を計算する際、ページ名を利用しないことで、精度を改善しました。（AN-140479）
* Workspace のパフォーマンスを改善するために、折れ線グラフのビジュアライゼーションの問題を修正しました。（AN-174878）
* ダウンロードされた .csv ファイルに UTF-8 エンコードが適用されない問題を修正しました。（AN-178393）
* Workspace プロジェクトのパフォーマンスが遅かった問題を修正しました。（AN-177710）
...
オープンソースエディションとコマースエディションのリリースノートは、[リリース情報](https://devdocs.magento.com/guides/v2.3/release-notes/bk-release-notes.html)ページに あります。
