...
| 2022年5月 | [Tableau でのクエリサービスを使用したオムニチャネルインサイトの分析と視覚化](https://experienceleague.adobe.com/docs/platform-learn/tutorials/queries/analyze-and-visualize.html?lang=ja) | ビデオ | チャーン分析の例を使用して、Adobe Experience Platform クエリサービスを外部データビジュアライゼーションツールで使用する方法を説明します。 | Experience Platform |
...
* **Cloud Manager**
   ...
   * Cloud Manager ユーザーインターフェイスを介して、本番稼働デプロイ手順を再実行します。
   * 本番稼働デプロイ手順を再実行するために、ビルド画像を再利用します。
...
| 2022年5月 | [コードのデプロイ](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/using-cloud-manager/deploy-code.html?lang=ja) | ビデオ | AEM as a Cloud Service の Cloud Manager パイプラインを使用してコードを本番環境にデプロイする方法を説明します。 | AEM CS、Cloud Manager |
...
| 機能 | 説明 |
| ------- | ----------- |
| [!UICONTROL キャンペーン] | すべての広告主は、以下の Beta 機能を使用できます。<ul><li>（[!DNL Microsoft® Advertising] アカウント）[!DNL Microsoft® Audience Network]（[!DNL Microsoft® Audience Ads] を含む）上の既存のネイティブ広告キャンペーンの同期、読み取り専用表示およびレポート（ビュースルーデータを含む）のサポート。</li><li>（[!DNL Google Ads] アカウントと[!DNL Microsoft® Advertising] アカウント）キャンペーンとキャンペーン構造を[!UICONTROL 検索] > [!UICONTROL &#x200B; ツール &#x200B;] > [!UICONTROL &#x200B; キャンペーンのインポート Beta]から[!DNL Microsoft® Advertising]にインポートできます。<br><br> キャンペーンをインポートすると、インポートジョブのステータスを確認したり、エラーログを確認したり、インポートスケジュールを編集、一時停止、削除したりできます。[!DNL Google Ads]</li></ul> |
