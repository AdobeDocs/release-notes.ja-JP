...
* **Adobe Experience Manager as a Cloud Service の基盤**
  ...
  * クラウド環境のカスタム機能に対する、より効率的で効果的なトラブルシューティングのために、アドビは、新しい開発者ツール、[リポジトリブラウザー](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/developer-tools/repository-browser.html?lang=ja)をリリースしました。 軽量な読み取り専用の HTML ブラウザーで、Developer Console から起動できます。 パブリッシャー層、オーサー層、プレビュー層、およびすべての環境（本番、ステージ、開発を含む）のコンテンツリポジトリを可視化します。 コンテンツ構造を参照したり、プロパティを表示したり、バイナリをプレビューおよびダウンロードしたりします。
...
* **Cloud Manager**
  ...
  * BannedPath [品質ルール](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/using-cloud-manager/test-results/code-quality-testing.html?lang=ja#understanding-code-quality-rules)の重大度が、blocker から critical に変更されました。
...
| 2022年4月 | [DDX 操作の呼び出しを使用した PDF のアセンブリ](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/forms/forms-cs-assembler/introduction.html?lang=ja) | ビデオ | リポジトリブラウザーは、AEM の基盤となるデータストアを可視化する強力なツールで、AEM as a Cloud Service 環境のデバッグを容易にします。 リポジトリブラウザーは、本番、ステージ、開発に加え、オーサー、パブリッシュおよびプレビューサービスの、AEM のすべてのリソースおよびプロパティの検査をサポートします。 | AEM CS |
| 2022年4月 | [レターインスタンスを保存および取得するための実装ソリューション](https://experienceleague.adobe.com/docs/experience-manager-learn/forms/store-retrieve-letters/introduction.html?lang=ja) | ビデオ | SPI を使用して、インタラクティブなコミュニケーションのレターインスタンスを保存および取得するための実装方法を説明します。 | AEM Forms |
...
| 2022年4月 | [監査記録](https://experienceleague.adobe.com/docs/campaign-learn/tutorials/monitoring/audit-trail.html?lang=ja) | ビデオ | 監査記録ログにアクセスする方法と設定可能な構成について説明します。 | Campaign v8 |
...
| 2022年4月 | [送信後のドキュメントの変更](https://experienceleague.adobe.com/docs/document-cloud-learn/sign-learning-hub/getting-started/getting-started-sending/modify-in-flight.html?lang=ja) | ビデオ | 誤って別のドキュメントを送信してしまった場合など、既に署名用に送信してしまったドキュメントを変更する方法を説明します。 | Adobe Sign |
