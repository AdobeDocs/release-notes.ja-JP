...
| 機能 | 説明 |
| ------- | ----------- |
| [!UICONTROL &#x200B; ポートフォリオ &#x200B;]、[!UICONTROL &#x200B; キャンペーン &#x200B;] | （オプトインベータ版の機能）新しい「[!UICONTROL ネットワーク支出の制約]」オプションを使用すると、マルチネットワークポートフォリオで、特定の広告ネットワークに対する予算の割り当てに関する制限を定義できます。このオプションは、日、週、月ごとの支出戦略を備えたポートフォリオで、「[!UICONTROL キャンペーンの最大支出ターゲット (％) を有効にする]」オプションは有効になっていません。「[!UICONTROL キャンペーン予算の制限を自動調整]」オプションが選択されている場合、複数および最小のキャンペーン予算設定に指定された値の範囲内で制約が機能します。<br><br>基本レポートの新しいネットワーク制約レポートには、ポートフォリオに指定された最小および最大の検索エンジン予算と実際の支出（コスト）がすべて表示されます。<br><br>ベータ版プログラムに参加するには、アカウントチームにお問い合わせください。 |
