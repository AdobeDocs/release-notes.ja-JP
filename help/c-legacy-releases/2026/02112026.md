...
*  [!DNL Experience Cloud]](#ai)の[AI機能（更新日：**3月3**）
...
| 公開日 | 名前 | タイプ | 説明 |
|---------|----|----|-----------|
| 2026年1月 | [ 有効期限のない資格情報に関するCustomer Journey Analytics BI 拡張機能の接続の問題 ](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29891) | 新しい記事 | Adobe Experience Platformで期限切れにならない資格情報に、BI ツールを操作するための正しいCustomer Journey Analytics アクセス権限とデータビュー権限が必要な場合の問題の解決策について説明します。 |
| 2026年1月 | [ スキーマ参照が無効なため、ストリーミング取り込みに失敗しました](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29710) | 新しい記事 | Adobe Experience Platformでペイロードのスキーマ参照が正しくないか、形式が正しくないため、ストリーミング取り込みが失敗した場合の問題の解決策について説明します。 |
| 2026年1月 | AEP](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29715)の期限切れのSAS トークンが原因で[[!DNL Databricks]  コネクタが失敗します | 新しい記事 | インフラストラクチャの変更後に[!DNL Databricks] コネクタが機能しなくなった場合、またはSpark設定のSAS （[!DNL Shared Access Signature]） トークンが期限切れになったり、AEPで更新されたりした場合の問題の解決策について説明します。 |
| 2026年1月 | [S3宛先への増分書き出しは、AEPでマッピングされたオーディエンスを書き出しません](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29725) | 新しい記事 | マッピングされたオーディエンスがAEPで有効な増分数を示しているにもかかわらず、Amazon S3宛先への増分書き出しがゼロのレコードになる場合の問題の解決策について説明します。 |
| 2026年1月 | [ マーケティングアクションリストがAdobe Experience Platformに表示されない](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29744) | 新しい記事 | 影響を受けるユーザーがAdobe Experience Platformで適切なデータガバナンス権限を持っていないため、広告宛先を設定する際にマーケティングアクションのリストが表示されず、ページが空白で表示される場合の問題の解決策について説明します。 |
| 2026年1月 | AEP](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29745)でソースフロー実行の[False遅延アラート | 新しい記事 | データフローの実行が正常に終了しても、ソースフローの実行の遅延に関するアラートが表示される問題の解決策について説明します。これは、アラートシステムの監視信号がAdobe Experience Platformの実際の実行指標と必ずしも一致しないためです。 |
| 2026年1月 | [Adobe Experience Platformでのストリーミングデータフロー取り込みエラーに関するアラートがトリガーされない](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29762) | 新しい記事 | Adobe Experience Platformのストリーミングデータフローでソースフローのエラーに関するアラートが機能しない場合の問題の解決策について説明します。 |
| 2026年1月 | AEP](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29774)で [!DNL LiveRamp] にセグメントを書き出すと[認証エラーが発生する | 新しい記事 | 実稼動用サンドボックスから[!DNL LiveRamp] オンボーディング先へのセグメントの書き出し中に認証エラーが発生した場合の問題の解決策について説明します。Adobe Experience Platformで。 |
| 2026年1月 | [ プロファイルがAEPのGoogle Customer Matchに同期されません](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29806) | 新しい記事 | Google Customer Matchにオーディエンスをアクティブ化する際に、プロファイルがGoogleでサポートされるID属性を持たないか、マッピングが正しくないため、プロファイルの小さなサブセットのみが正常に同期される場合の問題の解決策について説明します。 |
| 2026年1月 | [[!DNL Meta Pixel] 拡張機能は、AEPで同意の前にJavaScriptを読み込みます](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29840) | 新しい記事 | Adobe Experience Platformの[!DNL Meta Pixel]拡張機能が同意を提供する前に`fbevents.js` スクリプトを読み込む場合の問題の解決策について説明します。 |
| 2026年1月 | [Adobe Experience Platformと [!DNL The Trade Desk] 統合して [!DNL The Trade Desk ID]](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29847)を収集する | 新しい記事 | [!DNL The Trade Desk]とAdobe Experience Platformを統合する際に、Web SDKを通じて[!DNL The Trade Desk ID]を収集およびマッピングする際に不確かになる問題の解決策について説明します。 |
...
| 公開日 | 名前 | タイプ | 説明 |
| -----------| ---------- | ---------- | ---------- |
| 2026年2月 | [Adobe Analytics オンボーディング ](https://experienceleague.adobe.com/en/docs/analytics/analyze/admin-overview/onboard) | ビデオ | Adobe Adobe Analyticsをサポートする主要なAdobeプラットフォームにオンボーディングして、自社の成功を支援します。 |
...
| 2026年2月 | [CJAを選ぶ理由 CJAとAdobe Analyticsの戦略的レビュー](https://experienceleague.adobe.com/en/docs/customer-journey-analytics-learn/tutorials/cja-basics/why-cja-modernize-customer-analytics) | ビデオ | Adobe Experience Platform上に構築されたAdobe Customer Journey Analytics（CJA）が、従来のAdobe Analyticsの限界をどのように克服しているかをご確認ください。 |
| 2026年2月 | [ フリーフォームテーブルの複数のディメンション列](https://experienceleague.adobe.com/en/docs/customer-journey-analytics-learn/tutorials/analysis-workspace/visualizations/multiple-dimension-columns-in-freeform-tables) | ビデオ | CJAの複数のDimension列機能を使用すると、フリーフォームテーブルをさらに強力にできます。 |
| 2026年2月 | [Customer Journey AnalyticsのAnalysis Workspace プロジェクトに棒グラフのビジュアライゼーションを追加](https://experienceleague.adobe.com/docs/customer-journey-analytics-learn/tutorials/analysis-workspace/visualizations/add-bar-visualizations.html) | ビデオ | Customer Journey AnalyticsのAnalysis Workspace プロジェクトで、標準バーと積み重ね棒のビジュアライゼーションを追加および設定する方法について説明します。 |
...
| ----------| ---------- | ---------- | ---------- |---------- |
| 2026年2月 | [!DNL AEM CS] | [開発エージェントの概要](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/ai-in-aem/agents/development/overview) | ビデオ | AEMのDevelopment Agentが、Cloud Managerで失敗したパイプラインを分析し、コード修正を提案してデバッグを高速化するためのログをビルドする方法を説明します。 |
| 2026年2月 | [!DNL AEM Assets] | [Adobe スナップショット ](https://experienceleague.adobe.com/docs/experience-manager-learn/assets/dynamic-media/dynamic-media-snapshot.html) | ビデオ | Adobe Snapshotは、最適化されたダイナミックな画像配信を実現するDynamic Mediaの機能を示すビジュアルデモンストレーションツールです。 |
| 2026年2月 | [!DNL AEM CS] | [AEM as a Cloud Serviceでのコンテンツの復元](https://experienceleague.adobe.com/en/docs/experience-manager-learn/cloud-service/cloud-manager/content-restore) | ビデオ | AEM as a Cloud Serviceのコンテンツを復元する方法を、ステップバイステップガイドでご確認ください。 コンテンツの整合性を確保しながら、以前のコンテンツを効率的に復元するタイミングと方法を把握できます。 |
...
| 公開日 | 名前 | タイプ | 説明 |
|---------|--------|---------|---------|
| 2026年1月 | [DASH/HLS アダプティブストリーミングで&#x200B;*高度なストリーミングが有効になっていない* エラーがAEM Dynamic Media](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29657)で返される | 新しい記事 | DASH アダプティブストリーミングにアクセスすると、AEM Dynamic MediaのMPD ファイルのURL形式が正しくないため、*高度なストリーミングが有効になっていない*&#x200B;というエラーが返される問題の解決策について説明します。 |
| 2026年1月 | [Adobe Experience Manager：エクスペリエンスフラグメント（XF）のコピーとペーストの処理がエラーポップアップで失敗する](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29837) | 新しい記事 | エクスペリエンスフラグメント（XF）をコピーしてペーストすると、エラーポップアップが表示され、XFがペーストされない場合の問題の解決策について説明します。 |
...
| 2026年1月 | [AEMのJava バージョンが一致しないため、TCP Locatorを開始できません](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28132) | 新しい記事 | クラスタ環境のSP22 ホットフィックスの後にJDK 17以降が必要なため、TCP Locatorの起動に失敗した場合の問題の解決策について説明します。 |
| 2026年1月 | [ZIP アセットが404を返す（AEM as a Cloud Serviceの[!UICONTROL 公開]、またはDispatcherのAEM 6.5](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28174)） | 新しい記事 | AEM DAMにアップロードされたZIP ファイルが[!UICONTROL 作成者]に公開されたとして表示されますが、DispatcherがデフォルトでZIP リクエストをブロックするため、[!UICONTROL 公開]にアクセスすると404 エラーが返される問題の解決策について説明します。 |
| 2026年1月 | [AEM ログを [!DNL Nginx]](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28164)経由でHTTP エンドポイントに転送する際にエラーログが見つかりません | 新しい記事 | AEM ログを[!DNL Nginx]を使用してカスタム HTTP エンドポイントに転送する際に、HTTP エンドポイントが一時的に利用できないか、応答が遅いため、エラー例外ログが欠落する問題の解決策について説明します。 |
| 2026年1月 | [AEM as a Cloud ServiceでのAdobe Launch （Tags）環境マッピングについて](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28165) | 新しい記事 | AEM as a Cloud ServiceでAdobe Launchを設定する際の問題の解決策について説明します。ステージングタブと実稼動タブのみが表示され、各環境で使用されるLaunch ライブラリが混同される原因となります。 |
| 2026年1月 | [AEM as a Cloud Service パブリッシュポッド全体でDispatcher `vhost`設定に一貫性がありません](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29602) | 新しい記事 | 一部のAEM as a Cloud Service パブリッシュ ポッドが更新されたDispatcher `vhost`設定を適用しない場合、必要なホストが明示的に定義されていないため、*HTTP 301* リダイレクトと無限リダイレクトループが一貫して発生する問題の解決策について説明します。 |
| 2026年1月 | [ リダイレクトプロパティがAEM ライブコピーにロールアウトされない](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29707) | 新しい記事 | デフォルトのMSM アクションではリダイレクト関連のプロパティが除外されているため、ブループリントページで設定されたリダイレクト設定がロールアウト中にライブコピーに反映されない場合の問題の解決策について説明します。 |
| 2026年1月 | [AEMaaCS](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-28055)で正しいAdobe リポジトリを使用してEDS プロジェクトを設定する | 新しい記事 | AEMaaCSでEdge Delivery Services プロジェクトを設定する際に、正しいAdobe ボイラープレートリポジトリを使用する必要がある場合の問題の解決策について説明します。 |
...
| 2026年1月 | [[!DNL Site-Wide Analysis Tool]  レポートを開くことができません。エラー](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-27906)は、*400 Bad Request - Request Header Or Cookie Too Large*&#x200B;です。 | 新しい記事 | リクエストヘッダーまたはCookieがAdobe Commerce Cloudのサーバー設定制限を超えているため、[!DNL Site-Wide Analysis Tool] レポートが開かず、*400 Bad Request エラー*&#x200B;が表示される場合の問題の解決策について説明します。 |
| 2026年1月 | [Adobe Commerceの高度なレポートのトラブルシューティング ](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29603) | 新しい記事 | Adobe Commerceでの高度なレポートの問題の診断と解決について説明します。 |
| 2026年1月 | [[!DNL Cloudflare] Adobe Commerceでのタイムアウトとリダイレクトの問題](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29759) | 新しい記事 | Adobe Commerce環境で[!DNL Cloudflare]設定がタイムアウトとリダイレクトの問題を引き起こす場合の問題の解決策について説明します。 |
| 2026年1月 | [ データベースのアップロードが [!DNL MySQL]](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29843)への接続を失います | 新しい記事 | データベースのアップロードが[!DNL MySQL]への接続を失った場合の問題の解決策について説明します。 |
| 2026年1月 | [Adobe CommerceでImage Optimizationを有効にしているときにエラーが発生しました](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29890) | 新しい記事 | [!DNL Fastly]画像の最適化（[!DNL Fastly] IO）がデフォルトで無効になっており、画像の最適化を有効にするように[!DNL Fastly]に連絡する通知が表示されている場合の問題の解決策について説明します。 |
| 2026年1月 | [ クラウドインフラストラクチャー上のAdobe Commerceにおける `var/export` フォルダー権限の問題 ](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29895) | 新しい記事 | `var/export/email` フォルダー内のサーバーのファイル権限の問題により、製品データをエクスポートできない場合の問題の解決策について説明します。 |
...
| 公開日 | 名前 | タイプ | 説明 |
|---------|----|----|-----------|
| 2026年1月 | [Adobe Campaign Managed Cloudでのアクティブなプロファイル数の増加の修正](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29761) | 新しい記事 | 複数のターゲットマッピングが同じログスキーマを共有し、Adobe Campaign Managed Cloudでアクティブなプロファイル数が予期せず増加する場合の問題の解決策について説明します。 |
| 2026年1月 | [Campaign Standard：勝者が選択されている間、A/B テストの配信はまだ保留になっています](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29793) | 新しい記事 | すべてのバリエーションが受信者に配信される前にA/B テストが終了した場合の問題の解決策について説明します。 |
| 2026年1月 | [Campaign Classic: ワークフローをエクスポートすると、インスタンスがクラッシュする](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29792) | 新しい記事 | パッケージでカスタムキャンペーンワークフローを書き出すとインスタンスがクラッシュする場合の問題の解決策について説明します。 |
...
| ----------| ---------- | ---------- | ---------- |---------- |
| 2026年2月 | [!DNL Journey Optimizer] | [ ジャーニーを含むダイレクトメールメッセージを送信](https://experienceleague.adobe.com/docs/journey-optimizer-learn/tutorials/channels/direct-mail-channel/direct-mail-in-journeys.html) | ビデオ | Adobe Journey Optimizer のダイレクトメールチャネルを活用して、ジャーニー内のダイレクトメールの配信を自動化およびスケジュールする方法について説明します。 |
| 2026年2月 | [!DNL Journey Optimizer] | [AJO Decisioningでプッシュ通知を最適化](https://experienceleague.adobe.com/en/docs/journey-optimizer-learn/tutorials/decision-capabilities/decisioning/decisioning-in-push-notifications) | ビデオ | AJO の決定を使用して、オーディエンス向けにプッシュ通知をパーソナライズする方法について説明します。 受信者のプロファイルに基づいて魅力的なオファーを作成し、販売を推進します。 |
...
| 2026年2月 | [!DNL Journey Optimizer] | [SMS メッセージで決定を使用](https://experienceleague.adobe.com/en/docs/journey-optimizer-learn/tutorials/decision-capabilities/decisioning/use-decisioning-in-an-sms-message) | ビデオ | AJO Decisioningを使用してSMS コンテンツをパーソナライズし、最適化する方法を説明します。 エンゲージメントを向上させ、ビジネス目標を達成するためにSMS メッセージを強化します。 |
...
| 公開日 | 名前 | タイプ | 説明 |
|---------|----|----|-----------|
| 2026年1月 | [AJOでのイベントタイミングとバッチ取り込みにより、オーディエンスの選定が失敗します](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-29796) | 新しい記事 | Adobe Journey Optimizerのオーディエンス条件で定義された評価ウィンドウ外にイベントが存在するため、オーディエンスの選定が失敗した場合の問題の解決策について説明します。 |
...
| 公開日 | 名前 | タイプ | 説明 |
| -----------| ---------- | ---------- | ---------- |
| 2026年2月 | [GenStudio for Performance Marketing用Photoshop プラグイン ](https://experienceleague.adobe.com/en/docs/genstudio-for-performance-marketing/user-guide/integrations/photoshop-plugin) | ビデオ | GenStudio for Performance Marketing用Photoshop プラグインのインストール、設定、使用方法について説明します。 |
