...
* （CJA） レポートでCJAが通貨コードの代わりに通貨記号を使用するようになった問題を修正しました。（AN-268881）
...
* **[!UICONTROL ソース]:**&#x200B;ファイル取り込みカテゴリの名前がローカルシステムに変更され、ローカルファイルアップロードコネクタを使用してローカルファイルを直接 Platform に取り込めるようになりました。
...
* [Experience Leagueの最新のAdobe Experience Manager コンテンツの一覧| 2021年9月](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager/list-of-latest-adobe-experience-manager-content-on-experience/m-p/421751?profile.language=ja#M29908)
...
* [Experience Managerの新しい機能のリクエスト/提案を送信するには？](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager/announcing-the-new-process-to-submit-experience-manager-feature/td-p/380425?profile.language=ja)
...
| 機能 | 説明 |
| ------- | ----------- |
| 広告インサイト | （9月8日）Beta モードでは、追加のインサイトを利用できます。 |
| ブランドセーフティ | （9月22日リリース）[!DNL DoubleVerify] ブランドセーフティプレビッド製品が [!DNL Brand Suitability Tiers] に更新され、広告主は特定のトピックのすべてのインスタンスを避けることなく、特定のセグメントに対して 3 つのリスクレベル（低、中、高）を選択できるようになりました。 これまでは、セグメントに許容値のレベルが含まれていませんでした。 |
| — | 新しいベータ版 Advertising Cloud JavaScript タグは、Adobe Experience Cloud ID（ECID）サービスと従来の `ef_id` および `gsurferid` を使用して、コンバージョンを測定します。 この新しいタグは、[ファーストパーティ Experience Cloud s_ecid cookies](https://experienceleague.adobe.com/docs/core-services/interface/administration/ec-cookies/cookies-first-party.html?lang=ja) を作成し、他の Experience Cloud 製品とより緊密に統合できます。<br><br>ITP マッピングを備えた従来のAdvertising Cloud JavaScript タグは、2021年9月に非推奨となります。<br><br>Adobeのアカウントマネージャーから、詳細と実装手順について連絡を受けます。 |
